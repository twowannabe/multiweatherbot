...
def check_water_temperature():
    ...
    if current_temperature is not None:
        if previous_temperature is None:
            ...
            # Лучше использовать job_queue вместо asyncio.run
            job_queue.run_once(lambda _: asyncio.create_task(send_notification_to_all_users(message)), 0)
        previous_temperature = current_temperature
...
def schedule_solar_flare_check():
    schedule.every(12).hours.do(lambda: job_queue.run_once(lambda _: asyncio.create_task(send_solar_flare_forecast_to_all_users()), 0))
